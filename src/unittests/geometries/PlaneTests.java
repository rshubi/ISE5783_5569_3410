...
class PlaneTests {
	...
	void testPlanePointPointPoint() {
		...
		// TC11:Testing the construction of a plane when the three points are on the same line
	...
	void testGetNormalPoint() {
		...
		Plane p = new Plane(p1, p2, p3);
		Vector normal = p.getNormal(p1);
		assertEquals(1, normal.length(),0.00000001, "ERROR: the normal is not a unit vector");
		assertEquals(0, v1.dotProduct(normal), "ERROR:normal to plane is not correct");
		assertEquals(0, v2.dotProduct(normal), "ERROR:normal to plane is not correct");
		assertEquals(0, v3.dotProduct(normal), "ERROR:normal to plane is not correct");
	...
	public void testFindIntersections() {
		...
		Vector v2 = p2.subtract(p3);
		Vector v3 = p3.subtract(p1);
		// TC01:A ray that starts outside the plane, is not parallel to the plane, and makes a non-right angle with the plane and cuts it
		...
		// TC02:A ray that starts outside the plane, is not parallel to the plane, and makes a non-right angle with the plane and does not cut it
		...
		// 3 tests
		// TC13:A ray that starts in front of the plane, and is perpendicular to the plane
		Ray ray =new Ray(new Point(0.2, -0.5, 1),new Vector(2, 2, 2));
		...
		assertEquals(List.of(new Point(0.30000000000000004,-0.39999999999999997,1.1)),p.findIntersections(ray), "Ray's line out of sphere");
		...
		assertNull(p.findIntersections(ray), "Ray's line out of sphere");
		...
		assertNull(p.findIntersections(ray), "Ray's line out of sphere");
		// TC16:The beam starts inside the plane, and is neither perpendicular nor parallel to it.
		...
		assertNull(p.findIntersections(ray), "Ray's line out of sphere");
		// TC17:The beginning of the beam is exactly at the reference point of the plane.
		ray = new Ray(new Point(1, 0, 0), new Vector(0.5, 2.8, -1.5));
		assertNotEquals(0, ray.getDir().dotProduct(v1), "The ray is ortogonaly to the plane");
		assertNotEquals(0, ray.getDir().dotProduct(v2), "The ray is ortogonaly to the plane");
		assertNotEquals(0, ray.getDir().dotProduct(v3), "The ray is ortogonaly to the plane");
		
		assertNull(p.findIntersections(ray), "ERROR: must not be any intersections");
	}
