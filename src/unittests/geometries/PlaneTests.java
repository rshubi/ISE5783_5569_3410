...
class PlaneTests {
	...
	public void testFindIntersections() {
		...
		Point p3 = new Point(0, 0, 1);
		Plane p = new Plane(p1, p2, p3);
		GeoPoint p4 = new GeoPoint(p, new Point(0.833333333333333, 0.16666666666666696, 0.0));
		Vector v1 = p1.subtract(p2);
		Vector v2 = p2.subtract(p3);
		Vector v3 = p3.subtract(p1);
		...
		List<GeoPoint> result = p.findGeoIntersectionsHelper(r);
		...
		assertNull(p.findGeoIntersectionsHelper(new Ray(new Point(3, 1, -1), new Vector(2, 2, 2))),
				"Ray's line out of sphere");
		// =============== Boundary Values Tests ==================
		// 2 tests
		// TC11:A ray that starts outside the plane, and is parallel to the plane
		assertNull(p.findGeoIntersectionsHelper(new Ray(new Point(2, 2, 2), new Vector(-3, 0, 3))),
				"Ray's line out of sphere");
		// TC12:A ray inside the plane, and parallel to the plane
		assertNull(p.findGeoIntersectionsHelper(new Ray(new Point(1, 0, 0), new Vector(-3, 0, 3))),
				"Ray's line out of sphere");
		// 3 tests
		...
		result = p.findGeoIntersectionsHelper(ray);
		...
		assertEquals(List.of(new GeoPoint(p, new Point(0.30000000000000004, -0.39999999999999997, 1.1))),
				p.findGeoIntersectionsHelper(ray), "Ray's line out of sphere");
		...
		assertNull(p.findGeoIntersectionsHelper(ray), "Ray's line out of sphere");
		...
		assertNull(p.findGeoIntersectionsHelper(ray), "ERROR: must not be any intersections");
	}
